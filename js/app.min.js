...
1);b=new URLSearchParams(b);var c=parseInt(b.get("page"));a.page=isNaN(c)?1:Math.max(1,c);c=parseInt(b.get("sort"));a.sort=isNaN(c)?null:c;c=parseInt(b.get("record"));a.record=isNaN(c)?null:c;a.search=b.get("search");a.tag=b.get("tag");a.layout=b.get("layout");a.startDate=b.get("start_date");a.endDate=b.get("end_date")},timestampToDateString$$module$js$util$util=function(a){if(!a)return"";a=new Date(1E3*a);let b=("0"+(a.getUTCMonth()+1)).slice(-2),c=("0"+a.getUTCDate()).slice(-2);return`${a.getUTCFullYear()}-${b}-${c}`},
dateStringToTimestamp$$module$js$util$util=a=>Date.parse(a)/1E3,slug$$module$js$util$util=function(a){if(!a)return"";a=a.trim().toLowerCase().replace(/[^a-zA-Z]+/g,"|");if(!a)return"";a=a.split("|");return a.length?a.slice(0,10).filter(b=>2<b.trim().length).join("-"):""};String.prototype.capitalize=function(){return this.replace(/(^|\s)([a-z])/g,function(a,b,c){return b+c.toUpperCase()})};
...
module$js$util$util.isNotEmpty=isNotEmpty$$module$js$util$util;module$js$util$util.loadParams=loadParams$$module$js$util$util;module$js$util$util.meta_helper=meta_helper$$module$js$util$util;module$js$util$util.parseMoneyToDecimalString=parseMoneyToDecimalString$$module$js$util$util;module$js$util$util.record_change_helper=record_change_helper$$module$js$util$util;module$js$util$util.rest=rest$$module$js$util$util;module$js$util$util.slug=slug$$module$js$util$util;
module$js$util$util.timestampToDateString=timestampToDateString$$module$js$util$util;let toast_helper$$module$js$util$modal={toastTemplate:Handlebars.compile($("#toast-template").html()),show:function(a,b){$("#app-toast").html(this.toastTemplate({message:a,background:b}));(new bootstrap.Toast($("#toast"))).show()}},modal_helper$$module$js$util$modal={el:"#app-modal",m:function(){return bootstrap.Modal.getOrCreateInstance($(".modal",this.el)[0],{keyboard:!1,backdrop:"static"})},validate:function(a){a.fields.forEach(d=>{record_change_helper$$module$js$util$util.fieldInputs(d).forEach(e=>
...
rest$$module$js$util$util.post(document.datwik_api_url+"/auth/register",b).then(c=>{Login$$module$js$module$login.renderAuthentication(c);toast_helper$$module$js$util$modal.show(`Successfully registered and logged in ${b.username}`,"text-white bg-success");window.app.router().navigateTo("/")}).catch(c=>{toast_helper$$module$js$util$modal.show(`Error during registration: ${c.message}`,"text-white bg-danger")})})},init:function(a){this.el=a;this.render()},render:async function(){this.el.html(this.template({}));
...
"text-white bg-danger")})}})},init:function(a){this.el=a;this.render()},render:async function(){this.el.html(this.template({}));this.events()}};var module$js$module$profile={};module$js$module$profile.Profile=Profile$$module$js$module$profile;const ResetPassword$$module$js$module$reset_password={el:null,template:compile_template$$module$js$util$util("#reset-password-template"),events:function(){$("*[data-action='change-password']").off();$("*[data-action='change-password']").click(a=>{a.preventDefault();a=window.location.href.slice(window.location.href.indexOf("?")+1);a=(new URLSearchParams(a)).get("token");a={new_password:$("*[data-input='new-password']").val(),token:a};if(""==a.password||""==a.new_password)toast_helper$$module$js$util$modal.show("Please enter all values",
"text-white bg-danger");else{var b=$("*[data-input='new-password-repeat']").val();a.new_password!=b?toast_helper$$module$js$util$modal.show("Please enter same new password twice","text-white bg-danger"):rest$$module$js$util$util.post(document.datwik_api_url+"/auth/reset_password",a).then(c=>{toast_helper$$module$js$util$modal.show(`Successfully changed password for ${c.username}`,"text-white bg-success");window.app.router().navigateTo("/")}).catch(c=>{toast_helper$$module$js$util$modal.show(`Error during password change: ${c.message}`,
