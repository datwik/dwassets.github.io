...
{d:5,v:"Security/Vulnerability"},{d:6,v:"Enhancement"}],id:2},{name:"message",type:"TEXTAREA",id:3}]}},rest$$module$js$util$util={get:async function(a){var b=new Date;return axios({url:a,method:"get",headers:{}}).then(function(c){console.log("get",": ",new Date-b);return c.data}).catch(function(c){throw Error(c);})},_change:async function(a,b,c){var d=new Date;return axios({url:a,method:c,headers:{},data:b,withCredentials:!0}).then(function(e){console.log(c,": ",new Date-d);return e.data}).catch(e=>
{if(e.response&&e.response.data.m)throw Error(e.response.data.m);throw Error(e.message);})},post:async function(a,b){return this._change(a,b,"post")},put:async function(a,b){return this._change(a,b,"put")},delete:async function(a,b){return this._change(a,b,"delete")}};var module$js$util$util={};module$js$util$util.FieldType=FieldType$$module$js$util$util;module$js$util$util.buildParams=buildParams$$module$js$util$util;module$js$util$util.compile_template=compile_template$$module$js$util$util;
...
return!c&&a},setDefaultContextValues:function(a){a.okButtonText||(a.okButtonText="Save changes");return a},events:function(){$("*[data-url-id]").off();$("*[data-url-id]").blur(a=>{a=$(a.currentTarget);var b=a.attr("data-url-id");b=$(`input[data-url-value-id='${b}']`);let c=b.val();if(!(c&&0<c.length))try{let d=new URL(a.val()),e=d.hostname.replace(/(\.[^\.]*){1}$/,"").replace(/^([^\.]*\.){1}/,""),f=d.pathname.replace(/[\/\-_]+/g," "),g=(e+" "+f).replace(/\b\w/g,h=>h.toUpperCase());b.val(g);b.select()}catch(d){console.log(d)}})},
show:async function(a){a=this.setDefaultContextValues(a);let b=a.template(a),c=a.onConfirm,d=a.confirmToastMessage,e=this;$(this.el).html("");$(this.el).html(b);modal_helper$$module$js$util$modal.events();$("button[name='modal-confirm']").off();$(".modal",this.el).off();this.m().show();return(new Promise((f,g)=>{let h=$("button[data-action='modal-confirm']");h.click(k=>{k.preventDefault();if(!a.meta||e.validate(a.meta))h.addClass("disabled"),h.html('<i class="fas fa-spinner fa-pulse"></i>&nbsp;'+
